...
def vincenty_distance(lon1, lat1, lon2, lat2):
    ...
    while abs(lambda_val - lambda_old) > 1e-12 and itercount < 50:
        ...
        sin_sigma = math.sqrt((math.cos(U2)*math.sin(lambda_val))**2 + 
                             (math.cos(U1)*math.sin(U2) - math.sin(U1)*math.cos(U2)*math.cos(lambda_val))**2)
        cos_sigma = math.sin(U1)*math.sin(U2) + math.cos(U1)*math.cos(U2)*math.cos(lambda_val)
        sigma = math.atan2(sin_sigma, cos_sigma)
        
        if sin_sigma == 0:
            alpha = 0  # Points are coincident
        else:
            alpha = math.asin(math.cos(U1)*math.cos(U2)*math.sin(lambda_val)/sin_sigma)
        
        cos2_sigma_m = cos_sigma - 2*math.sin(U1)*math.sin(U2)/(math.cos(alpha)**2)
        C = f/16 * math.cos(alpha)**2 * (4 + f*(4 - 3*math.cos(alpha)**2))
        
        lambda_old = lambda_val
        lambda_val = L + (1-C)*f*math.sin(alpha)*(sigma + C*sin_sigma*(cos2_sigma_m + 
                     C*cos_sigma*(-1 + 2*cos2_sigma_m**2)))
    ...
    if abs(lambda_val) < 1e-12:  # Same point
        ...
        denom = math.cos(U1)*math.sin(U2) - math.sin(U1)*math.cos(U2)*math.cos(lambda_val)
        azimuth_forward = math.degrees(math.atan2(numer, denom)) % 360
        
        # Backward azimuth  
        numer = math.cos(U1) * math.sin(lambda_val)
        denom = math.sin(U1)*math.cos(U2) - math.cos(U1)*math.sin(U2)*math.cos(lambda_val)
...
def vincenty_forward(lon1, lat1, azimuth, distance):
    ...
    while itercount < 50:
        ...
        delta_sigma = B*math.sin(sigma)*(cos2_sigma_m + B/4*(math.cos(sigma)*(-1 + 
                     2*cos2_sigma_m**2) - B/6*cos2_sigma_m*(-3 + 4*math.sin(sigma)**2)*
                     (-3 + 4*cos2_sigma_m**2)))
    ...
    denom = math.cos(U1)*math.cos(sigma) - math.sin(U1)*math.sin(sigma)*math.cos(azimuth_rad)
    lambda_val = math.atan2(numer, denom)
    
    C = f/16 * math.cos(alpha)**2 * (4 + f*(4 - 3*math.cos(alpha)**2))
    L = lambda_val - (1-C)*f*math.sin(alpha)*(sigma + C*math.sin(sigma)*
    ...
    numer = math.sin(U1)*math.cos(sigma) + math.cos(U1)*math.sin(sigma)*math.cos(azimuth_rad)
    denom = (1-f) * math.sqrt(math.sin(alpha)**2 + 
            (math.sin(U1)*math.sin(sigma) - math.cos(U1)*math.cos(sigma)*math.cos(azimuth_rad))**2)
    lat2 = math.degrees(math.atan2(numer, denom))
    
    # Backward azimuth
    azimuth_back = math.degrees(math.atan2(-math.sin(alpha), 
                   math.sin(U1)*math.sin(sigma) - math.cos(U1)*math.cos(sigma)*math.cos(azimuth_rad))) % 360
