...
def solve_anchor_position(triang_data: dict, 
                         true_sound_speed: float = 1503,
                         measured_sound_speed: float = 1507) -> dict:
    """
    ...
    for i, (lat, lon, range_val) in enumerate(zip(
        triang_data['latitudes'], triang_data['longitudes'], corrected_ranges)):
    ...
    anchor_lon, anchor_lat = _solve_trilateration(positions, horizontal_ranges, lon0, lat0)
    
    # Calculate residuals
    residuals = _calculate_residuals(positions, horizontal_ranges, anchor_lon, anchor_lat)
    ...
    fallback_distance, _, _ = vincenty_distance(deploy_lon, deploy_lat, anchor_lon, anchor_lat)
...
def _solve_trilateration(positions: List[Tuple[float, float]], 
                        ranges: List[float],
                        lon0: float, lat0: float) -> Tuple[float, float]:
    """
    ...
    anchor_lon, anchor_lat, _ = vincenty_forward(lon0, lat0, correction_azimuth, correction_dist)
    
    return anchor_lon, anchor_lat


def _calculate_residuals(positions: List[Tuple[float, float]], 
                        ranges: List[float],
                        anchor_lon: float, anchor_lat: float) -> np.ndarray:
    """
...
def iterative_solve_anchor_position(triang_data: dict,
                                   true_sound_speed: float = 1503,
    ...
    solution = solve_anchor_position(triang_data, true_sound_speed, measured_sound_speed)
    ...
    for iteration in range(max_iterations - 1):
        ...
        position_change = vincenty_distance(
            previous_solution['anchor_lon'], previous_solution['anchor_lat'],
            solution['anchor_lon'], solution['anchor_lat']
        )[0]
