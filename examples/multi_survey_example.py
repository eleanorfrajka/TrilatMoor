...
from trilatmoor import plot_multiple_surveys, plot_multiple_solutions, process_survey_file
...
def main():
    ...
    if not survey_files:
        print("No survey data files found. Please add some data files to the data/ directory.")
        return
    ...
    try:
        ...
        for loc_name, triang_data, solution in survey_results:
            print(f"  {loc_name}: RMS {solution['rms_residual']:.1f}m, "
    ...
    if survey_files:
        ...
        triang_data2, solution2 = process_survey_file(survey_files[0], sound_speed=1480)  # Custom
        
        print(f"Default sound speed:")
        print(f"  Position: {solution1['anchor_lat']:.6f}°N, {solution1['anchor_lon']:.6f}°W")
        print(f"  RMS: {solution1['rms_residual']:.1f}m")
        
        print(f"Custom sound speed (1480 m/s):")
        print(f"  Position: {solution2['anchor_lat']:.6f}°N, {solution2['anchor_lon']:.6f}°W")
