...
from trilaterate_moor import solve_anchor_position, dec2deg, load_bathymetry_netcdf_subsampled
...
def create_comprehensive_single_plot(all_data, bathymetry_path=None):
    ...
    print(f"Bathymetry region: {bathy_west:.2f}W to {bathy_east:.2f}W, {bathy_south:.2f}N to {bathy_north:.2f}N")
    ...
    if bathymetry_path and os.path.exists(bathymetry_path):
        try:
            ...
            plot_bathymetry(ax, bathy, [bathy_west, bathy_east], [bathy_south, bathy_north])
            print(f"Loaded bathymetry: {len(bathy['lon'])} x {len(bathy['lat'])} points")
    ...
    for i, (loc_name, triang_data, solution) in enumerate(all_data):
        ...
        deploy_marker = ax.plot(deploy_lon, deploy_lat, 's', color=color, markersize=8, 
                               markeredgecolor='black', markeredgewidth=1)
        
        # Plot final anchor position
        anchor_lat = solution['anchor_lat']
        anchor_lon = solution['anchor_lon']
        anchor_marker = ax.plot(anchor_lon, anchor_lat, '*', color=color, markersize=12,
                               markeredgecolor='black', markeredgewidth=1)
        
        # Plot fallback line
        fallback_line = ax.plot([deploy_lon, anchor_lon], [deploy_lat, anchor_lat], 
        ...
        x = np.cos(lat1_rad) * np.sin(np.radians(anchor_lat)) - np.sin(lat1_rad) * np.cos(np.radians(anchor_lat)) * np.cos(dlon_rad)
        ...
        if 'G' in loc_name:  # dsG1, dsG2, dsG3 - bottom edge at 65°36.0'N
            ...
            x_pos = -29.55 + (g_num - 1) * lon_range / 3 + lon_range / 6  # Evenly spaced
            ...
            lon_range = -29.15 - (-29.55)  # 0.4 degrees
            x_pos = -29.55 + (k_num - 1) * lon_range / 3 + lon_range / 6  # Evenly spaced
    ...
    if len(g_moorings) >= 3:
        ...
        for i in range(len(g_names)):
            for j in range(i + 1, len(g_names)):
                ...
                print(f"Drawing line from {name1} ({lon1:.4f}, {lat1:.4f}) to {name2} ({lon2:.4f}, {lat2:.4f})")
                ax.plot([lon1, lon2], [lat1, lat2], 'b--', alpha=0.8, linewidth=2.0, 
    ...
    ax.text(-29.117, 65.61, "Gross", fontsize=12, fontweight='bold',
           horizontalalignment='center', verticalalignment='center')
    
    # "Klein" at 65°28.2'N, 29°7'W  
    ax.text(-29.117, 65.47, "Klein", fontsize=12, fontweight='bold',
           horizontalalignment='center', verticalalignment='center')
    ...
    # Title
    ax.set_title('Trilateration Survey - MIXSED array\n'
...
def main():
    ...
    print("="*90)
    print(f"{'Location':<8} {'Final Position (°N, °W)':<25} {'Fallback':<10} {'RMS':<8} {'Quality':<10}")
    print("-" * 90)
    ...
    for loc_name, triang_data, solution in all_data:
        ...
        print(f"{loc_name:<8} {pos_str:<25} {fallback:<10.0f} {rms:<8.1f} {quality:<10}")
