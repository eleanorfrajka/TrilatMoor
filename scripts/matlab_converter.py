...
def convert_matlab_ship_track(mat_filename: str, output_filename: str = None):
    ...
    if not HAS_SCIPY:
        raise ImportError(
            "scipy required for reading MATLAB files. Install with: pip install scipy"
        )
    ...
    if "ship" in mat_data:
        ...
        print(
            "Available variables:",
            [k for k in mat_data.keys() if not k.startswith("_")],
        )
        return None
    ...
    print(
        f"Lat range: {np.min(ship_data['lat']):.4f} to {np.max(ship_data['lat']):.4f}"
    )
    print(
        f"Lon range: {np.min(ship_data['lon']):.4f} to {np.max(ship_data['lon']):.4f}"
    )
    ...
    if output_filename and HAS_XARRAY:
        ...
        print(
            "Warning: xarray not available, cannot save NetCDF. Install with: pip install xarray"
        )
...
def create_example_ship_track():
    ...
    lats = (
        base_lat + radius * np.cos(angles) + np.random.normal(0, noise_level, n_points)
    )
    lons = (
        base_lon + radius * np.sin(angles) + np.random.normal(0, noise_level, n_points)
    )

    track_data = {"time": times, "lat": lats.tolist(), "lon": lons.tolist()}
