...
def create_comprehensive_plot(all_data, bathymetry_path=None):
    ...
    ax_main.set_title(
        "Comprehensive Trilateration Survey Summary", fontsize=16, fontweight="bold"
    )
    ...
    for i, (loc_name, triang_data, solution) in enumerate(all_data):
        ...
        # Plot survey fixes
        for j, (lat, lon, range_val) in enumerate(
            zip(
                triang_data["latitudes"][1:],
    ...
    # Individual plots for each survey
    for i, ((loc_name, triang_data, solution), ax) in enumerate(
        zip(all_data[: len(axes_individual)], axes_individual)
    ):
        try:
            # Use the existing plot function for individual surveys
            temp_fig = plot_trilateration_survey(
                triang_data, solution, bathymetry=bathy
            )
            ...
            # Deployment
            ax.plot(
                triang_data["longitudes"][0],
                triang_data["latitudes"][0],
                "s",
                color="red",
                markersize=8,
                label="Deployment",
            ...
            # Survey fixes
            for lat, lon in zip(
                triang_data["latitudes"][1:], triang_data["longitudes"][1:]
            ):
...
def main():
    ...
    print("=" * 80)
    print(
        f"{'Location':<12} {'Lat (°N)':<12} {'Lon (°W)':<12} {'RMS (m)':<10} {'Quality':<10}"
    )
    print("-" * 80)
